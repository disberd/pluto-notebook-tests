...
# ╔═╡ 50e4b2b0-cb8b-11eb-3d0a-6d6be6e02563
begin
	import Pkg
...
function parameters_table(dict;head_name="Parameters",kwargs...)
	...
	out = @htl("""
		<table style="margin=2em; align-self: center; flex: 0 0 12em;">
			$out
		</table>
		""")
...
function table_merge(table_dict;names=keys(table_dict),button=nothing)
	...
	for k ∈ names
		out = @htl("""
				...
				$out $(table_dict[k]) <span style="flex: 0 0 1em;"></span>

			""")
	end
	# Put this in a separate div
	out = @htl("""
		<div style="display: flex; align-items: flex-start;align-content: center overflow: auto;">
			$out
		</div>
		""")
	if button !== nothing
